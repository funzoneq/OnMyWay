...
tag = `/usr/bin/ipmitool fru print | grep 'Asset Tag' | awk '{ print $5 }' | head -n1`
...
if not client.exists?(tag) or asset.get_attribute("reintake") == 'true'
  ...
  client.set_multi_attribute!(tag, { "lldp" => lldp, "lshw" => lshw })
end
